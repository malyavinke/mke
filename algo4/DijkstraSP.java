...
public class DijkstraSP {
	...
	public static boolean wanted(int value, int [] arr){
		for (int v: arr){
			if ((v-1) == value) return true;
		}
		return false;
	}
	...
	public static void main(String[] args) {
		In in = new In(args[0]);
		EdgeWeightedDigraph G = new EdgeWeightedDigraph(in, Integer.parseInt(args[2]));
		int s = Integer.parseInt(args[1]);

		// compute shortest paths
		DijkstraSP sp = new DijkstraSP(G, s);

		int [] a = new int[]{7,37,59,82,99,115,133,165,188,197};
		// print shortest path
		for (int t = 0; t < G.V(); t++) {
			if (wanted(t, a)){
				if (sp.hasPathTo(t)) {
				//	StdOut.printf("%d to %d (%.2f)  ", s, t, sp.distTo(t));
				//	StdOut.print("%d", (int)sp.distTo(t));
					System.out.print((int)sp.distTo(t) + ",");
					if (sp.hasPathTo(t)) {
