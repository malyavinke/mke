...
def get_age(age):
    try:
        age = float(age) #No fare recorded will come up as a string so
...
def get_fare(fare):
    try:
        fare = float(fare) #No fare recorded will come up as a string so
                                    #try to make it a float
    except: #If fails then just bin the fare according to the class
        fare = 100
    if fare <= 10:
        ...
        for datum in line:
            ...
            if i in attr_list:
                if i == 4:
                    if not datum: 
                        values.append('100')
                    else: values.append(get_age(datum))
                elif i==8: values.append(get_fare(datum))
                else: 
    ...
    for line in data:
        ...
        for datum in line:
            ...
            if i in attr_list:
                ...
                elif i==7: values.append(get_fare(datum))
...
#    print attributes
    new_attributes = [attributes[0], attributes[1], attributes[3],attributes[8]]
#    print new_attributes
    ...
    tree = create_decision_tree(get_data(csv_file_object, new_attributes, [0,1,3,8]), new_attributes, target_attr, gain)
#    tree = create_decision_tree(get_data(csv_file_object, new_attributes, [0,1,3,4,5,6,8,10]), new_attributes, target_attr, gain)
...
#    target_attr = attributes[0]
    ...
    classification = classify(tree, get_test_data(csv_file_object,new_attributes, [0,2,7]))
